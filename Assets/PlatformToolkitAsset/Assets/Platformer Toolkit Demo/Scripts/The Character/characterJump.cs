...
namespace GMTK.PlatformerToolkit {
    ...
    public class characterJump : MonoBehaviour {
        ...
        [Tooltip("How long the character hangs at the jump apex")]
        public float apexHangTime = 0.1f;
        public float apexGravMultiplier = 0.1f;
        
        [SerializeField, Range(1f, 20f)]
        [Tooltip("How quickly gravity fades out at the apex")]
        public float apexGravityFadeOutSpeed = 10f;
        ...
        private void calculateGravity() {
            ...
            if (apexHangCounter > 0f) {
                gravMultiplier =  Mathf.MoveTowards(gravMultiplier, apexGravMultiplier, apexGravityFadeOutSpeed * Time.deltaTime);
                body.velocity = new Vector3(velocity.x, Mathf.Clamp(velocity.y, -speedLimit, 100));
                return; // skip all other gravity logic while hanging
            }
            
            if (gravMultiplier < downwardMovementMultiplier && !onGround && body.velocity.y < -0.01f) {
                gravMultiplier = Mathf.MoveTowards(gravMultiplier, downwardMovementMultiplier, apexGravityFadeInSpeed * Time.deltaTime);
                body.velocity = new Vector3(velocity.x, Mathf.Clamp(velocity.y, -speedLimit, 100));
                return;
            }
